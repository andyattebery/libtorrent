...
#define SIGNAL_BITFIELD_DID_INTERNAL_ERROR(verify_slot, did_throw)      \
  CPPUNIT_ASSERT(verify_did_internal_error(bind(&torrent::signal_bitfield::add_signal, \
                                                     &signal_bitfield,  \
...
utils_signal_bitfield_test::test_basic() {
  ...
  for (unsigned int i = 0; i < torrent::signal_bitfield::max_size; i++)
    CPPUNIT_ASSERT(signal_bitfield.add_signal(bind(&mark_index, &marked_bitfield, i)) == i);

  SIGNAL_BITFIELD_DID_INTERNAL_ERROR(bind(&mark_index, &marked_bitfield, torrent::signal_bitfield::max_size), true);
}
...
utils_signal_bitfield_test::test_single() {
  ...
  CPPUNIT_ASSERT(signal_bitfield.add_signal(bind(&mark_index, &marked_bitfield, 0)) == 0);
...
utils_signal_bitfield_test::test_thread() {
  ...
  for (unsigned int i = 0; i < torrent::signal_bitfield::max_size; i++)
    CPPUNIT_ASSERT(thread->signal_bitfield()->add_signal(bind(&mark_index, &marked_bitfield, i)) == i);
  ...
  for (int i = 0; i < 100; i++) {
    ...
    CPPUNIT_ASSERT(wait_for_true(bind(&check_index, &marked_bitfield, i % 20)));
    __sync_fetch_and_and(&marked_bitfield, ~uint32_t());
  }

  thread->stop_thread();
  CPPUNIT_ASSERT(wait_for_true(bind(&thread_test::is_state, thread, thread_test::STATE_INACTIVE)));
